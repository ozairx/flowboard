...
class CardService {
  async createCard(title: string, listId: number, order: number, description?: string): Promise<Card> {
  ...
  async reorderCards(listId: number, cardUpdates: { id: number; order: number; listId: number }[]): Promise<void> {
